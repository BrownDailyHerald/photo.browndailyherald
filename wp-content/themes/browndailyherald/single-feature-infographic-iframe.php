<?php
/*
Template Name Posts: Featured Infographic (IFrame) Template
*/
?>
<?php get_header(); ?>
...
$attachment_info = get_post(get_post_thumbnail_id());
$image_attributes = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
$title_attr = '';
if (empty($attachment_info->post_excerpt)) {
	$title_attr = strip_tags(get_media_credit_html(get_post_thumbnail_id()));
}

$infographic_url = get_post_meta($post->ID, 'wpzoom_post_embed_code', true);

?>
			<div class="post-cover">
				<a href="<?php echo $infographic_url ?>" title="<?php echo esc_attr($title_attr); ?>" rel="post-featured-image" class="fancybox-iframe"><?php echo wp_get_attachment_image(get_post_thumbnail_id(), 'singlepost'); ?></a>
				<p class="image-byline"><?php echo get_media_credit_html(get_post_thumbnail_id()); ?></p>
<?php if (!empty($attachment_info->post_excerpt)): ?>
				<p class="image-caption"><?php echo esc_html($attachment_info->post_excerpt); ?></p>
<?php endif; ?>
			</div>
...
?>
<?php if ($post_thumbnail_id == $attachment->ID) continue; ?>
