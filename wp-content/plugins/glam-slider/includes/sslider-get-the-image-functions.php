 ...
 * is found, call glam_sslider_display_the_image() to format it.
 *
 * @since 0.1
 ...
 */
function glam_sslider_get_the_image( $args = array() ) {
	global $post;
	...
	$args = apply_filters( 'glam_sslider_get_the_image_args', $args );
	...
	extract( $args );
		...
		if ( $custom_key )
			$image = glam_sslider_image_by_custom_field( $args );
 ...
 * @return array|bool
 */
function glam_sslider_image_by_custom_field( $args = array() ) {
 ...
 * other filters are later added in the glam_sslider_display_the_image() function.
 *
 * @since 0.4
 * @param array $args
 * @return array|bool
 */
function glam_sslider_image_by_the_post_thumbnail( $args = array() ) {
 ...
 * @return array|bool
 */
function glam_sslider_image_by_attachment( $args = array() ) {
...
/**
 * Scans the post for images within the content.  Not called by default with glam_sslider_get_the_image().
 ...
 * @return array|bool
 */
function glam_sslider_image_by_scan( $args = array() ) {
 ...
 * given in an array.  Not used with glam_sslider_get_the_image() by default.
 *
 * @since 0.3
 * @param array $args
 * @return array
 */
function glam_sslider_image_by_default( $args = array() ) {
 ...
 */
function glam_sslider_display_the_image( $args = array(), $image = false ) {
	...
	if($timthumb=='1'){
	  $html = '<img src="'.glam_slider_plugin_url( 'includes/timthumb.php' ) .'?src='. $image['url'] . '&amp;h='.$h.'&amp;w='.$w.'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" class="' . esc_attr( $class ) . '"' . $width . $height . $style .' />';	  
    }
	else{
	...
	if ( $link_to_post ) {
		$html = '<a href="' . get_permalink( $post_id ) . '" '.$a_attr.'>' . $html . '</a>';}
	else {
	    if($permalink!='') {
	      $html = '<a href="' . $permalink . '" '.$a_attr.'>' . $html . '</a>';
		}
	}
...
/**
 * Get the image with a link to the post.  Use glam_sslider_get_the_image() instead.
 *
 * @since 0.1
 * @deprecated 0.3
 */
function glam_sslider_get_the_image_link( $deprecated = '', $deprecated_2 = '', $deprecated_3 = '' ) {
	glam_sslider_get_the_image();
}
