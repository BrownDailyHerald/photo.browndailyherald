...
function glam_get_slider_posts_in_order($slider_id) {
    global $wpdb, $table_prefix;
...
function get_glam_slider_name($slider_id) {
    global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_META;
...
function glam_ss_get_post_sliders($post_id){
    global $wpdb,$table_prefix;
...
function glam_ss_post_on_slider($post_id,$slider_id){
    global $wpdb,$table_prefix;
...
function glam_ss_slider_on_this_post($post_id){
    global $wpdb,$table_prefix;
	$slider_postmeta = $table_prefix.GLAM_SLIDER_POST_META;
    $sql = "SELECT * FROM $slider_postmeta  
	        WHERE post_id = '$post_id';";
...
function glam_slider($post_id,$slider_id = '1') {
	global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_TABLE;
...
function is_post_on_any_glam_slider($post_id) {
	global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_TABLE;
...
function is_glam_slider_on_slider_table($slider_id) {
	global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_TABLE;
...
function is_glam_slider_on_meta_table($slider_id) {
	global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_META;
	$check = "SELECT * FROM $table_name WHERE slider_id = '$slider_id' LIMIT 1;";
	$result = $wpdb->query($check);
...
function is_glam_slider_on_postmeta_table($slider_id) {
	global $wpdb, $table_prefix;
...
function get_glam_slider_for_the_post($post_id) {
    global $wpdb, $table_prefix;
	$table_name = $table_prefix.GLAM_SLIDER_POST_META;
...
function glam_slider_word_limiter( $text, $limit = 50 ) {
...
function glam_sslider_admin_url( $query = array() ) {
	global $plugin_page;
...
function glam_slider_table_exists($table, $db) { 
...
function glam_get_google_font($font=''){
