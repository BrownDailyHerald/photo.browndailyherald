...
add_action('admin_menu', 'wpzoom_options_box');
	...
	function wpzoom_options_box() {
		...
		add_meta_box('wpzoom_post_layout', 'Post Layout', 'wpzoom_post_layout_options', 'video', 'normal', 'high');
		add_meta_box('wpzoom_post_options', 'Post Options', 'wpzoom_post_info', 'post', 'side', 'high');
 		add_meta_box('wpzoom_post_embed', 'Embed a Video', 'wpzoom_post_embed_info', 'video', 'normal', 'high');
 	}
...
function wpzoom_post_info() {
	...
	<fieldset>
		<p>
			...
			<input type="checkbox" name="wpzoom_is_featured" id="wpzoom_is_featured" value="1" <?php echo $isChecked; ?> /> <label for="wpzoom_is_featured">Feature this Post?</label>
		</p>
 	</fieldset>
	<?php
}
...
/* Video embedding box for Video posts */
function wpzoom_post_embed_info() {
	...
	<fieldset>
		...
		<p class="wpz_border" style="border-bottom:none; padding:0;">
			<strong>Embed Video</strong> (<em>YouTube, Vimeo, etc.</em>):<br />
				<textarea name="wpzoom_post_embed_code" id="wpzoom_post_embed_code"><?php echo get_post_meta($post->ID, 'wpzoom_post_embed_code', true); ?></textarea><br/>
 		</p>
